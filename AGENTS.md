...
- **Agent takımı:** İndeks `ekip/TAKIM.md` (kurallar + kartlar + görev tahtası); her agentın kendi kartı `ekip/agent-*.md`. Ritüel: oturum başında AGENTS.md + TAKIM.md + KENDİ kartını oku; kapanmadan kendi kartını güncelle (context ölürse dosyadan devam). Bölgeler: SEO/içerik/reklam = bu oturum (`ekip/agent-pazarlama-seo.md`); `css/`, `tailwind.config.js`, `package.json` = Yazılım oturumu (`ekip/agent-yazilim-dev.md`). Eski `KOORDINASYON.md` stub'tır.

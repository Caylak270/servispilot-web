...
- **OFİS MOTORU:** Claude Code CLI 2.1.273 + GLM 5.3 Flash (chelper ile bağlı). 🔴 BLOKER (17 Eyl, 2 farklı anahtarla doğrulandı): model çağrısı "Credit balance is too low" — anahtarlar geçerli, sorun hesap/plan seviyesinde. Kontrol: Z.ai panelinde GLM Coding Plan aboneliği aktif mi + kalan kredi? Anahtar coding plan sayfasından mı alınmış? Entegrasyon testi stub motorla uçtan uca PASS (issue→lease→worktree→doğrulama→PR). Zamanlayıcılar kurulu ama DISABLED (OfisAjan 30dk + OfisAjanBaslangic) — kredi gelince ENABLE.
